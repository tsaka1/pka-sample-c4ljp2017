...
n_pass = cgi["newpwd"]
if n_pass != cgi["vrfy"]
  redirect_to(cgi, $key_chg_uri)
  log.info($0) { "password type-miss, redirect to #{$key_chg_uri}" }
else
  salt = nil
  pass = nil
  status = nil
  ...
  begin
	...
	begin
	  db.transaction{
	    db.execute("update pwd set salt = ?, passwd = ? where email = ?;",
	      salt, pass, u_name)
	  }
	rescue SQLite3::BusyException
	  sleep(0.1)
	...
	log.info($0) { "change password OK, redirect to #{$home_uri}" }
      else
	redirect_to(cgi, $key_chg_uri)
	log.info($0) { "old-password miss, redirect to #{$key_chg_uri}" }
      end
    else
