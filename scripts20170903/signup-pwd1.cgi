...
begin
  if u_name !~ /^[^@]+@[^@]+$/ || u_pass !~ /^sakaguchi/i
    ...
    begin
      db.transaction{
	db.execute("delete from pwd where email = ?;", u_name)
	db.execute("insert into pwd (email, salt, passwd, status) \
...
From: #{$from_email}
To: #{u_name}
Subject: pka4 - PIN for Sign-up / Password reset
...
admin of #{$home_uri}
    ...
    cgi.out(
      ...
      <html>
      <head>
      <title>Sign-Up / Password reset, 2nd step.</title>
      </head>
      <body>
      <h1>Sign-Up / Password reset, 2nd step.</h1>
      <form method="POST" action="signup-pwd2.cgi">
