...
begin
  begin
    db.transaction{
      db.execute("update pka set status = 'locked' where status = 'reset' \
	  and tstamp < datetime('now', '-24 hours');")
      db.execute("select salt, pubkey from pka where email = ? \
  ...
  if !salt || !pass
    ...
    cgi.out({
      ...
      <html>
      <head>
      <title>Reset Keypair</title>
      <script type="text/javascript" src="jsrsasign-all-min.js"></script>
      </head>
      <body>
      <h1>Reset Keypair of #{esc_tags(u_name)}</h1>
      <form method="POST" action="signup-pka3.cgi" onsubmit="return gen_key()">
	Passphrase (for private key): <input name="newpass" type="password" size="20" /><br/>
	Passphrase (confirm): <input name="vrfy" type="password" size="20" /><br/>
	<input name="pin" type="hidden" value="#{u_pin}" />
	<input name="email" type="hidden" value="#{u_name}" />
	<input name="pubkey" type="hidden" value="" />
	<input type="submit" />
	<input type="reset" />
      </form>
      <hr/>
      <p><a href="#{$home_uri}">Return to Home</a>.</p>
      <script type="text/javascript" src="signup-pka2.js"></script>
      </body>
      </html>
      EOS
    }
